...
class JobSolutionController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($tab === 'exams') {
            $data['exams'] = PastExam::with('organization', 'examCategory')
                ->withCount('questions')
                ->latest('exam_date')
            ...
            $data['organizations'] = Organization::withCount('pastExams')
                ->orderBy('name')
    ...
    public function organizationShow(Request $request, $organizationSlug)
    {
        ...
        $query = PastExam::where('organization_id', $organization->id)
            ->with('examCategory')
            ->withCount('questions');
        ...
        $years = PastExam::where('organization_id', $organization->id)
            ->selectRaw('YEAR(exam_date) as year')
            ->distinct()
            ->orderBy('year', 'desc')
            ->pluck('year');

        return view('frontend.job-solutions.organization-show', compact('organization', 'exams', 'categories', 'years', 'search', 'categoryId', 'year', 'type'));
    }

    public function show($organizationSlug, $examSlug)
    {
        $organization = Organization::where('slug', $organizationSlug)->firstOrFail();
        ...
        // Fetch other top organizations (excluding current one)
        $otherOrganizations = Organization::where('id', '!=', $organization->id)
            ->withCount('pastExams')
        ...
        return view('frontend.job-solutions.show', compact(
            'exam', 'organization', 'subjectsData', 'activeQuestions', 'topicWeightage', 'totalQuestions', 'otherOrganizations'
        ));
    }
    ...
    public function questionShow($slug)
    {
        ...
        if ($question->subject_id) {
            $chapterStats = Chapter::where('subject_id', $question->subject_id)
                ->withCount('questions')
                ->get();
        }
