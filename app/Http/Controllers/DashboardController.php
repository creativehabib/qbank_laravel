    ...
    public function __invoke(Request $request): View|RedirectResponse
    {
        ...
        if ($user->isAdmin()) {
            ...
            if (\Schema::hasTable('question_reports')) {
                $criticalAlerts = QuestionReport::query()
        ...
        $lastDaysModelTests = ModelTestResult::with('modelTest')
        ...
        $lastModelTest = ModelTestResult::where('user_id', $userId)->latest('created_at')->first();
        ...
        $modelExamsList = ModelTestResult::with('modelTest')
            ->where('user_id', $userId)
            ->latest('created_at')
            ->take(5)
            ->get()
