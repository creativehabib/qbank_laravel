...
class OmrGenerator extends PageController
{
    ...
    public function mount(): void
    {
        ...
        if ($user && $user->hasRole('teacher')) {
            ...
            // অপশন ১: আপনার সিস্টেমে যদি User মডেলের সাথে Organization রিলেশন থাকে (যেমন: $user->organization)
            if (isset($user->organization)) {
                $this->schoolName = $user->organization->name ?? $this->schoolName;
                $this->address = $user->organization->address ?? $this->address;
            }
            // অপশন ২: যদি সরাসরি ইউজার টেবিলেই স্কুল/প্রতিষ্ঠানের কলাম থাকে
            else {
                $this->schoolName = $user->organization_name ?? $user->school_name ?? $this->schoolName;
                $this->address = $user->organization_address ?? $user->address ?? $this->address;
            }
        }
    }
    ...
    {
        return view('pages.omr-generator')->layout('layouts.app', [
            'title' => 'OMR Generator',
        ]);
    }
}
