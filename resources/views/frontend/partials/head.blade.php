...
@php
    ...
    // Global SEO Settings
    $seo = \App\Support\SettingsStore::group('seo');
    $defaultMetaTitle = $seo['meta_title'] ?? $appName;
    $defaultMetaDesc = $seo['meta_description'] ?? $siteDesc;
    $metaKeywords = $seo['meta_keywords'] ?? 'qbank, questions, job preparation';
    $twitterHandle = $seo['twitter_handle'] ?? '@qerobi';

    $ogImageSetting = $seo['og_image'] ?? null;
    $defaultOgImage = !empty($ogImageSetting) ? asset('storage/'.$ogImageSetting) : asset('images/og-image.png');

    // SEO Meta Tags Dynamic Setup
    $ogImage = $ogImage ?? $defaultOgImage;
    $isHome = request()->routeIs('home') || request()->is('/');
    if ($isHome) {
        $pageTitle = filled($title ?? null) ? $defaultMetaTitle .' | '.$title : $defaultMetaTitle;
    } else {
        $pageTitle = filled($title ?? null) ? $title .' | '.$defaultMetaTitle : $defaultMetaTitle;
    }
    $pageDesc = filled($description ?? null) ? $description : $defaultMetaDesc;
...
@if(!empty($twitterHandle))<meta name="twitter:site" content="{{ $twitterHandle }}">@endif
...
<!-- 1. Dark Mode Init (Zero-Flash) - Moved to top -->
<script>
    (function() {
        try {
...
<!-- 2. Schema.org JSON-LD Structured Data (Fixed for Laravel Blade with @@) -->
...
@if(!empty($tracking['google_analytics_id']))
    <!-- 3. Google Analytics (GA4) with Zero-Performance-Impact -->
    <link rel="dns-prefetch" href="https://www.googletagmanager.com">
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
        gtag('config', '{{ $tracking['google_analytics_id'] }}', { send_page_view: true });
        ...
        (function() {
            ...
            function loadGtag() {
                ...
                script.async = true;
                script.src = 'https://www.googletagmanager.com/gtag/js?id={{ $tracking['google_analytics_id'] }}';
                document.head.appendChild(script);
                cleanupListeners();
            }

            const events = ['pointerdown', 'keydown', 'scroll', 'touchstart'];
            function onInteraction() { loadGtag(); }

            function cleanupListeners() {
                events.forEach(function(e) { window.removeEventListener(e, onInteraction, { passive: true }); });
            }

            events.forEach(function(e) { window.addEventListener(e, onInteraction, { passive: true, once: true }); });

            if ('requestIdleCallback' in window) {
                window.addEventListener('load', function() { requestIdleCallback(loadGtag, { timeout: 3500 }); });
            } else {
                window.addEventListener('load', function() { setTimeout(loadGtag, 3500); });
            }
        })();
    </script>
@endif
