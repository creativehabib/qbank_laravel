                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                @if(is_array($options))
                                    @foreach($options as $index => $optionData)
                                        ...
                                        @endphp
                                        <div class="min-h-[3.5rem] flex items-center gap-2 rounded-lg border px-3 py-2 {{ $isCorrect ? 'border-emerald-300 bg-emerald-50 dark:border-emerald-600 dark:bg-emerald-900/20' : 'border-zinc-200 bg-white dark:border-zinc-600 dark:bg-zinc-800' }}">
                    ...
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-5">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xs font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-2">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> {{ $question->subject->name ?? 'বিষয়' }} এর অধ্যায়সমূহ
                            </h3>
