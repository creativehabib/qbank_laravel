...
@section('title', $organization->name . ' এর সকল পরীক্ষার প্রশ্ন ও সমাধান')
@section('description', $organization->name . ' কর্তৃক পরিচালিত বিগত সালের সকল পরীক্ষার প্রশ্ন ও পূর্ণাঙ্গ সমাধান।')
                ...
                <div class="flex-grow w-full">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <div class="flex items-center gap-3 mb-2">
                                <span class="bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-0.5 rounded text-[10px] font-bold">নিয়োগকারী প্রতিষ্ঠান</span>
                                @if($organization->established_year)
                                    <span class="text-[11px] text-zinc-500 font-medium">প্রতিষ্ঠিত: {{ $organization->established_year }}</span>
                                @endif
                            </div>
                            <h1 class="text-2xl md:text-3xl font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
                                {{ $organization->name }} @if($organization->short_name) <span class="text-zinc-400 font-normal">({{ strtoupper($organization->short_name) }})</span> @endif
                            </h1>
                        </div>
                    ...
                    <div x-data="{ expanded: false }">
                        <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-3 leading-relaxed max-w-4xl transition-all duration-300 ease-in-out overflow-hidden" :class="expanded ? 'max-h-[1000px]' : 'max-h-[46px] line-clamp-2'">
                            {{ $organization->description ?? $organization->name . ' কর্তৃক পরিচালিত বিগত সালের সকল পরীক্ষার প্রশ্ন ও সমাধান নিচে দেওয়া হলো।' }}
                        </p>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
                <div>
                    <h2 class="text-lg font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-2 mb-1">
                        ...
                        অনুষ্ঠিত নিয়োগ ও প্রশ্ন সমাধান তালিকা (<span x-text="counts[type]"></span>)
                    </h2>
                    <p class="text-xs text-zinc-500">{{ $organization->name }} কর্তৃক পরিচালিত বিগত সালের সকল পরীক্ষার পূর্ণাঙ্গ সমাধান</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 flex items-center justify-center min-h-[250px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl">
                        <p class="text-zinc-500 dark:text-zinc-400 font-medium text-[13px] md:text-sm">এই প্রতিষ্ঠানের কোনো পরীক্ষা এখনো যুক্ত করা হয়নি।</p>
                    </div>
                @endforelse
                ...
                @if($allCount > 0)
                    <div x-show="counts[type] === 0" x-cloak class="col-span-1 md:col-span-2 flex items-center justify-center min-h-[250px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl">
                        <p class="text-zinc-500 dark:text-zinc-400 font-medium text-[13px] md:text-sm">এই ক্যাটাগরিতে কোনো পরীক্ষা পাওয়া যায়নি।</p>
                    </div>
                @endif
            </div>
        </div>
