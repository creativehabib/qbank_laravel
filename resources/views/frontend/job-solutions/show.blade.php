...
@section('title', $exam->title . ' - প্রশ্ন ও সমাধান')
@section('description', $exam->title . ' এর পূর্ণাঙ্গ প্রশ্ন ও নির্ভুল সমাধান।')
@if($exam->thumbnail)
@section('og_image', asset('storage/' . $exam->thumbnail))
@endif
                    ...
                    <div x-data="{ expanded: false}">
                        <div class="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed mb-4 transition-all duration-300 ease-in-out overflow-hidden" :class="expanded ? 'max-h-[1000px]' : 'max-h-[46px] line-clamp-2'">
                            @if($exam->description)
                                {!! $exam->description !!}
                            @else
                                {{ $organization->name }} এর {{ $exam->title }} পদের নিয়োগ পরীক্ষাটি সরকারি চাকরিপ্রার্থীদের জন্য একটি গুরুত্বপূর্ণ পরীক্ষা। এই পরীক্ষায় সাধারণত {{ $exam->type }} পদ্ধতিতে প্রশ্ন এসে থাকে। নিচে সম্পূর্ণ সমাধান দেওয়া হলো।
                            @endif
                        </div>
                ...
                <div class="flex items-center justify-between gap-3 sticky top-[64px] md:top-[72px] z-40 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-2.5 shadow-sm transition-all overflow-x-auto no-scrollbar [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    ...
                    <div class="flex shrink-0 bg-slate-50 dark:bg-zinc-800/50 p-1 rounded-full border border-slate-200 dark:border-zinc-700">
                        <button @click="isQuizMode = false; showAnswers = true; showExplanations = false"
                            ...
                            <span class="md:hidden">পড়া</span>
                            <span class="hidden md:inline">পড়ার মোড</span>
                        </button>
                    ...
                    <button @click="showTopicWeightage = !showTopicWeightage" class="w-full flex items-center justify-between p-4 text-left focus:outline-none">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-zinc-500 mt-0.5">কোন বিষয় ও টপিক থেকে কত শতাংশ প্রশ্ন এসেছে দেখতে ক্লিক করুন</p>
                            </div>
                        </div>
                ...
                <div class="space-y-6" x-ref="questionsContainer">
                                ...
                                @foreach($groupQs as $question)
                                    <div class="question-card relative z-10 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 md:p-5 hover:border-emerald-300 dark:hover:border-emerald-800 transition-colors shadow-sm">
                                        ...
                                        <div class="text-[15px] md:text-lg font-bold text-zinc-900 dark:text-zinc-100 leading-snug mb-4">
                                            ...
                                            <span class="text-[10px] md:text-xs font-normal text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded ml-1 align-middle inline-block">[{{ $organization->name }}]</span>
                                        </div>
                    ...
                    @empty
                        <div class="text-center py-12 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                            <p class="text-zinc-500">কোন প্রশ্ন পাওয়া যায়নি।</p>
                        </div>
                    @endforelse
            ...
            <div class="lg:col-span-4 space-y-6 sticky top-[92px] h-max">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
                    <div class="p-5 flex items-center gap-4">
                        @if($organization->logo_path)
                            <div class="w-12 h-12 rounded-full border border-zinc-100 flex items-center justify-center p-1 shadow-sm shrink-0">
                                <img src="{{ $organization->logo_url }}" alt="Logo" class="w-full h-full object-contain rounded-full">
                            </div>
                        @else
                        ...
                        @endif
                        <div>
                            <p class="text-[11px] font-bold text-emerald-600 mb-0.5">নিয়োগকারী প্রতিষ্ঠান</p>
                            <h3 class="text-base font-bold text-zinc-900 dark:text-zinc-100">{{ $organization->name }}</h3>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-white dark:bg-zinc-900">
                        <a href="{{ route('organization.show', $organization->slug) }}" class="block w-full py-2.5 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 text-center text-xs font-bold rounded-lg transition-colors border border-emerald-100 dark:border-emerald-800/50">
                            {{ $organization->short_name ?? 'Ministry' }} এর সকল পরীক্ষা &rarr;
                        </a>
                    </div>
                </div>

                <!-- Box 2: Other Top Organizations -->
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        @forelse($otherOrganizations as $otherInst)
                            <a href="{{ route('organization.show', $otherInst->slug) }}" class="flex items-center justify-between p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group">
                                <div class="flex items-center gap-3">
                                    @if($otherInst->logo_path)
                                        <div class="w-8 h-8 rounded-full border border-zinc-100 flex items-center justify-center p-0.5 shrink-0 bg-white">
                                            <img src="{{ $otherInst->logo_url }}" alt="Logo" class="w-full h-full object-contain rounded-full">
                                        </div>
                                    @else
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-zinc-400 mt-0.5">নিয়োগ পরীক্ষা</p>
                                    </div>
                                </div>
                    ...
                    init() {
                        ...
                        const observer = new IntersectionObserver((entries) => {
                            if (entries[0].isIntersecting) {
                                if (!this.isLoadingMore && this.hasMore && !this.isSwitchingTab) {
                                    this.loadMore();
                                }
                            }
                        }, {
