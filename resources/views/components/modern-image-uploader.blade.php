...
<div class="flex flex-col">
    ...
    @endif
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center p-4 text-center">
                <div class="size-8 rounded-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center mb-3 shadow-sm">
                    <x-ui.icon icon="arrow-down-tray" class="size-4 text-zinc-500 dark:text-zinc-400" />
                </div>
        ...
        <div class="flex items-center gap-2 mt-2">
            <input type="file" id="{{ $inputId }}" class="hidden" data-page-model="{{ $uploadModel }}" accept="image/*">

            <x-ui.button type="button" size="sm" variant="subtle" icon="pencil-square" onclick="document.getElementById('{{ $inputId }}').click()">
                Replace
            </x-ui.button>

            <x-ui.button type="button" size="sm" variant="subtle" icon="trash" data-page-click="$set('{{ $existingModel }}', null); $set('{{ $uploadModel }}', null)">
                Remove
            </x-ui.button>
        </div>
    @else
