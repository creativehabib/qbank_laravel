<div class="flex items-center gap-2 w-full p-1 rounded-lg bg-zinc-100/50 dark:bg-zinc-800/50 border border-zinc-200/50 dark:border-zinc-700/50">
    ...
    @if(!auth()->user()->hasActiveSubscription() && !auth()->user()->isAdmin())
        <a href="{{ route('student.pricing') }}" class="in-data-flux-sidebar-collapsed-desktop:hidden shrink-0 text-xs font-bold bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white px-2 py-1.5 rounded-md border border-zinc-200 dark:border-zinc-700 shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-700 transition">
            Upgrade
        </a>
    @endif
</div>
