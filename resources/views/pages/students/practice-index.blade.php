<div x-data="{ filterOpen: false }" @keydown.escape.window="if(filterOpen) { filterOpen = false; } else if(document.activeElement.tagName !== 'INPUT') Page.('back')" class="relative flex flex-col lg:flex-row gap-5 lg:gap-6">

    <div class="w-full min-w-0 lg:flex-1">

        <div class="mb-6">
            {{ app(\App\Http\Controllers\Pages\Students\AiMagicSearch::class)->render() }}
        </div>
        ...
        <div class="space-y-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900 relative">
            ...
            <div data-page-loading.remove="hidden" class="hidden absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-white/60 backdrop-blur-sm dark:bg-zinc-900/60">
                <div class="flex flex-col items-center gap-2">
                    <x-ui.icon.arrow-path class="size-8 animate-spin text-emerald-600" />
            ...
            <div class="border-b border-zinc-200 dark:border-zinc-700">
                <div class="grid grid-cols-2">
                    <button type="button" data-page-click="$set('activeTab', 'fast')" class="border-b-2 px-4 py-3 text-base font-semibold cursor-pointer {{ $activeTab === 'fast' ? 'border-emerald-600 text-zinc-900 dark:text-zinc-100' : 'border-transparent text-zinc-400 hover:text-zinc-600 dark:text-zinc-500' }}">
                        {{ __('দ্রুত অনুশীলন') }}
                    </button>
                    <button type="button" data-page-click="$set('activeTab', 'mock')" class="border-b-2 px-4 py-3 text-base font-semibold cursor-pointer {{ $activeTab === 'mock' ? 'border-emerald-600 text-zinc-900 dark:text-zinc-100' : 'border-transparent text-zinc-400 hover:text-zinc-600 dark:text-zinc-500' }}">
                        {{ __('Mock Test') }}
                    </button>
                </div>
            </div>
                ...
                @if($activeTab === 'mock')
                    <div class="space-y-6 py-4">
                        <div class="text-center">
                            <x-ui.icon.academic-cap class="mx-auto mb-3 size-12 text-emerald-500" />
                        ...
                        <div class="mx-auto max-w-lg space-y-5 rounded-xl border border-zinc-200 bg-zinc-50 p-6 dark:border-zinc-700 dark:bg-zinc-800/50">
                            ...
                            <div class="space-y-2">
                                ...
                                <select data-page-model.live="selectedClassId" class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-100">
                            ...
                            @if($selectedClassId)
                                <div class="space-y-2">
                                    ...
                                    <select data-page-model.live="selectedSubjectId" class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-100">
                                        <option value="">{{ __('নির্বাচন করুন') }}</option>
                            ...
                            @if($selectedClassId)
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 border-t border-zinc-200 pt-5 dark:border-zinc-700">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <select data-page-model.live="questionCount" class="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-100">
                                    ...
                                    <div class="space-y-2 flex flex-col justify-center">
                                        ...
                                        <label class="relative inline-flex cursor-pointer items-center mt-1">
                                            <input type="checkbox" data-page-model.live="hasNegativeMark" class="peer sr-only">
                    ...
                    @if($level === 'filtered-questions')
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    <x-ui.icon.x-mark class="size-4" /> মুছুন
                                </button>
                            </div>
                            ...
                            @if($filteredQuestions->isEmpty())
                                <div class="rounded-lg border border-dashed border-zinc-300 p-8 text-center text-sm text-zinc-500 dark:border-zinc-700 dark:text-zinc-400">
                                    <x-ui.icon.folder-open class="mx-auto mb-2 size-8 text-zinc-300 dark:text-zinc-600" />
                            ...
                            @else
                                            ...
                                            <div x-data="{ openDescription: false }" class="mt-4 border-t border-zinc-200/60 pt-3 dark:border-zinc-700/60 space-y-3">
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                    ...
                                                    <div class="flex items-center gap-4 text-zinc-400 dark:text-zinc-500">
                                                        ...
                                                        <button type="button" data-page-click="toggleBookmark({{ $question->id }})" class="cursor-pointer {{ $question->is_bookmarked ? 'text-emerald-600 dark:text-emerald-400' : 'hover:text-emerald-600 dark:hover:text-emerald-400' }}" title="{{ $question->is_bookmarked ? 'Remove Bookmark' : 'Save Bookmark' }}">
                                                            <x-ui.icon.bookmark class="size-[18px]" variant="{{ $question->is_bookmarked ? 'solid' : 'outline' }}" />
                                                        </button>
                                                        <button type="button" data-page-click="toggleLike({{ $question->id }})" class="flex items-center gap-1 cursor-pointer {{ $question->is_liked ? 'text-pink-500' : 'hover:text-pink-500' }}" title="{{ $question->is_liked ? 'Unlike' : 'Like' }}">
                                ...
                                <div class="pt-6 pb-2 text-center" x-intersect.full="$page.loadMore()">
                                    @if($filteredQuestions->hasMorePages())
                                        <div data-page-loading data-page-target="loadMore" class="inline-flex items-center gap-2 text-zinc-500 font-medium text-sm">
                    ...
                    @else
                        @if($level === 'classes')
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                @foreach($classes as $class)
                                    <button type="button" data-page-click="openClass({{ $class->id }})" class="flex items-center justify-between cursor-pointer gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-left hover:border-emerald-500 hover:bg-emerald-50/40 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-emerald-500/60">
                                        <div class="flex items-center gap-3">
                                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300"><x-ui.icon.folder class="size-5" /></div>
                                            <div>
                                        ...
                                        <x-ui.icon.chevron-right class="size-5 shrink-0 text-zinc-400" />
                                    </button>
                                @endforeach
                            </div>
                        @endif
                        ...
                        @if($level !== 'classes')
                            <div class="flex items-center gap-2 text-sm">
                                <button type="button" data-page-click="back" class="rounded-md p-1 text-zinc-700 hover:bg-zinc-100 dark:text-zinc-200 dark:hover:bg-zinc-800" title="Back (Esc)"><x-ui.icon.arrow-left class="size-5" /></button>
                                <div class="flex flex-wrap items-center gap-1.5 text-zinc-500 dark:text-zinc-400">
                                    @if($selectedClassName)
                                        <span class="cursor-pointer font-medium text-zinc-700 hover:text-emerald-600 dark:text-zinc-200 dark:hover:text-emerald-400" data-page-click="$set('level', 'classes'); $set('selectedClassId', null); $set('selectedSubjectId', null); $set('selectedChapterId', null);">{{ $selectedClassName }}</span>
                                    @endif
                                    @if($selectedSubjectName)
                                        <x-ui.icon.chevron-right class="size-3.5 text-zinc-400" />
                                        @if($level === 'chapters' || ($level === 'questions' && $selectedChapterId))
                                            <span class="cursor-pointer font-medium text-zinc-700 hover:text-emerald-600 dark:text-zinc-200 dark:hover:text-emerald-400" data-page-click="$set('level', 'subjects'); $set('selectedSubjectId', null); $set('selectedChapterId', null);">{{ $selectedSubjectName }}</span>
                                        @else
                                ...
                                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                    @foreach($subjects as $subject)
                                        <div class="group flex items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 hover:border-emerald-500 hover:bg-emerald-50/40 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-emerald-500/60">
                                            ...
                                            <div data-page-click="openSubject({{ $subject->id }})" class="flex flex-1 items-center gap-3 cursor-pointer">
                                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300"><x-ui.icon.book-open class="size-5" /></div>
                                                <div>
                                            ...
                                            <div class="flex shrink-0 items-center">
                                                <button type="button" data-page-click="startSubjectPractice({{ $subject->id }})" class="hidden items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 group-hover:flex dark:bg-emerald-500 dark:hover:bg-emerald-600">
                                                    <x-ui.icon.play class="size-3.5" /> Start
                                                </button>
                                                <x-ui.icon.chevron-right class="size-5 text-zinc-400 group-hover:hidden" />
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                            ...
                                            <article class="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/70">
                                                ...
                                                <div x-data="{ openDescription: false }" class="mt-4 border-t border-zinc-200/60 pt-3 dark:border-zinc-700/60 space-y-3">
                                                    ...
                                                    <div x-show="openDescription" x-collapse x-cloak class="rounded-xl border border-dashed border-zinc-300 p-5 dark:border-zinc-600 mt-3">
                                                        ...
                                                        @else
                                                            <div class="space-y-3 text-center">
                                                                <div data-page-loading.remove data-page-target="generateAiExplanation({{ $question->id }})">
                                                                    <x-ui.icon.sparkles class="mx-auto size-6 text-violet-500" />
                                                                    <p class="font-semibold text-zinc-600 dark:text-zinc-300">{{ __('No explanation yet') }}</p>

                                                                    <button
                                                                        type="button"
                                                                        data-page-click.prevent="generateAiExplanation({{ $question->id }})"
                                                                ...
                                                                <div data-page-loading data-page-target="generateAiExplanation({{ $question->id }})">
                                                                    <div class="flex flex-col items-center gap-2">
                                                                        <x-ui.icon.arrow-path class="size-6 animate-spin text-violet-600" />
    ...
    @if($level === 'questions' || $level === 'filtered-questions')
        ...
        >
            <x-ui.icon.adjustments-horizontal class="size-6" />
            @if($level === 'filtered-questions')
        ...
        >
            ...
            <div class="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-200 bg-white/80 backdrop-blur-md p-4 dark:border-zinc-700 dark:bg-zinc-900/80">
                <h3 class="text-lg font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                    <x-ui.icon.adjustments-horizontal class="size-5 text-emerald-600" />
                    ফিল্টার
                    @if($level === 'filtered-questions')
            ...
            <div class="p-5 {{ $level === 'filtered-questions' ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : 'bg-white dark:bg-zinc-900' }} transition-colors duration-300">
                ...
                <div class="max-h-[calc(100vh-320px)] space-y-5 overflow-y-auto pr-1">
                    <div x-data="{ open: true }">
                        <button @click="open = !open" class="flex w-full items-center justify-between text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                            প্রশ্নের ধরন
                            <x-ui.icon.chevron-down class="size-4 transition-transform" x-bind:class="open ? 'rotate-180' : ''" />
                        </button>
                        <div x-show="open" x-collapse x-cloak class="mt-3 space-y-3">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" value="mcq" data-page-model.live="filterQuestionTypes" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" value="cq" data-page-model.live="filterQuestionTypes" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="checkbox" value="mcq+cq" data-page-model.live="filterQuestionTypes" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                    ...
                    <div x-data="{ open: true }">
                        ...
                        <div x-show="open" x-collapse x-cloak class="mt-3 space-y-3 max-h-40 overflow-y-auto pr-1">
                            @foreach($filterOptions['classes'] as $id => $name)
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" value="{{ $id }}" data-page-model.live="filterClasses" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                    ...
                    <div x-data="{ open: true }">
                        ...
                        <div x-show="open" x-collapse x-cloak class="mt-3 space-y-3 max-h-40 overflow-y-auto pr-1">
                            @foreach($filterOptions['subjects'] as $id => $name)
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" value="{{ $id }}" data-page-model.live="filterSubjects" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                                    <span class="text-sm text-zinc-600 group-hover:text-zinc-900 dark:text-zinc-300 dark:group-hover:text-zinc-100">{{ $name }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div x-data="{ open: true }">
                        ...
                        <div x-show="open" x-collapse x-cloak class="mt-3 space-y-3 max-h-40 overflow-y-auto pr-1">
                            @foreach($filterOptions['teachers'] as $id => $name)
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" value="{{ $id }}" data-page-model.live="filterTeachers" class="h-4 w-4 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500 dark:border-zinc-600" />
                                    <span class="text-sm text-zinc-600 group-hover:text-zinc-900 dark:text-zinc-300 dark:group-hover:text-zinc-100">{{ $name }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="mt-6 border-t border-zinc-200 pt-4 dark:border-zinc-700">
                    ...
                    >
                        <x-ui.icon.arrow-path class="size-4" />
                        সব ফিল্টার মুছুন
                    </button>
                </div>
            </div>
    ...
    {{-- ─── 🚀 ALPINE + UI CUSTOM REPORT ERROR MODAL ─── --}}
    <div
        x-data="{
            showModal: false,
            questionId: null,
