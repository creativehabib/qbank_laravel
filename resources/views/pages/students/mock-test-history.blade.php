<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <x-ui.button href="{{ route('students.practice.index') }}" variant="primary" icon="plus">
            নতুন পরীক্ষা দিন
        </x-ui.button>
    </div>
    ...
    @if($histories->isEmpty())
        <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-white py-20 dark:border-zinc-700 dark:bg-zinc-900">
            <x-ui.icon.document-magnifying-glass class="mb-4 size-16 text-zinc-300" />
            ...
            @foreach($histories as $history)
                <div class="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:border-violet-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-violet-900/50">
                    <div class="flex flex-col h-full">
                        ...
                        <div class="mt-auto">
                            @if($history->status === 'completed')
                                <x-ui.button href="{{ route('student.mock-test.result', ['testId' => $history->id]) }}" variant="filled" class="w-full !bg-zinc-900 dark:!bg-zinc-100 !text-white dark:!text-black" size="sm">
                                    রেজাল্ট দেখুন
                                </x-ui.button>
                            @else
                                <x-ui.button href="{{ route('student.mock-test.take', ['testId' => $history->id]) }}" variant="outline" class="w-full" size="sm">
                                    আবার শুরু করুন
                                </x-ui.button>
                            @endif
                        </div>
