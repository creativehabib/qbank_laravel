    ...
    <div class="grid grid-cols-1 items-start gap-6 lg:grid-cols-12">
        ...
        <div class="space-y-6 lg:col-span-8 xl:col-span-9">
                    ...
                    <div class="flex w-full flex-1 flex-col gap-4">
                        ...
                        <div class="flex items-center justify-between rounded-lg bg-[#f8fdfa] p-4 border border-emerald-50 dark:bg-emerald-900/10 dark:border-emerald-900/20">
                            ...
                            <div class="flex size-8 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30">
                                <x-ui.icon.arrow-right class="size-4" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 border-b border-zinc-200 border-dashed pb-6 dark:border-zinc-800">
                ...
                <button data-page-click="setFilter('wrong')"
                ...
                <button data-page-click="setFilter('right')"
                        class="rounded-md border px-5 py-2 text-xs font-bold tracking-wide transition
                    {{ $filter === 'right'
                ...
                <button data-page-click="setFilter('skipped')"
                        class="rounded-md border px-5 py-2 text-xs font-bold tracking-wide transition
                    {{ $filter === 'skipped'
            ...
            <div data-page-loading.class="opacity-50 pointer-events-none" class="transition-opacity duration-200">
                @if($questions->isEmpty())
                    <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-300 bg-white py-16 dark:border-zinc-700 dark:bg-zinc-900">
                        <x-ui.icon.document-magnifying-glass class="mb-4 size-12 text-zinc-300" />
                ...
                @else
                    <div class="space-y-4">
                        @foreach($questions as $question)
                            ...
                            <article class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900" data-page-key="mistake-{{ $question->id }}">
                                ...
                                <div class="mt-5 flex items-center justify-between border-t border-zinc-100 pt-4 dark:border-zinc-800">
                                    ...
                                    <button type="button" @click="openDescription === {{ $question->id }} ? openDescription = null : openDescription = {{ $question->id }}" class="flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400">
                                        DES
                                        <x-ui.icon.chevron-down class="size-3.5 transition-transform" x-bind:class="openDescription === {{ $question->id }} ? 'rotate-180' : ''" />
                                    </button>

                                    <div class="flex items-center gap-4 text-zinc-400">
                                        <div class="flex items-center gap-1 text-xs"><x-ui.icon.eye class="size-4" /> {{ $question->views_count ?? rand(100, 999) }}</div>
                                        <x-ui.icon.chart-pie class="size-4 cursor-pointer hover:text-zinc-600" />
                                        <x-ui.icon.bookmark class="size-4 cursor-pointer hover:text-zinc-600" />
                                        <x-ui.icon.heart class="size-4 cursor-pointer hover:text-red-500" />
                                        <x-ui.icon.flag class="size-4 cursor-pointer hover:text-zinc-600" />
                                        <x-ui.icon.share class="size-4 cursor-pointer hover:text-zinc-600" />
                                    </div>
                                </div>
                                    ...
                                    @else
                                        <div class="text-center">
                                            <div data-page-loading.remove data-page-target="generateAiExplanation({{ $question->id }})">
                                                <button type="button" data-page-click.prevent="generateAiExplanation({{ $question->id }})" class="inline-flex items-center gap-2 rounded-full bg-violet-600 px-4 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-violet-700">
                                                    ✨ AI Generate Explanation
                                                </button>
                                            </div>
                                            <div data-page-loading data-page-target="generateAiExplanation({{ $question->id }})" class="text-xs font-medium text-violet-600 animate-pulse">
        ...
        <div class="lg:col-span-4 xl:col-span-3">
            <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div class="mb-5 flex items-center justify-between">
                    ...
                    <x-ui.icon.arrow-right class="size-4 text-zinc-400" />
                </div>
                                ...
                                <button @click="activeTab = activeTab === {{ $index }} ? null : {{ $index }}" class="flex w-full items-center justify-between p-3.5">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex size-5 items-center justify-center rounded-full bg-zinc-100 text-zinc-500 transition-transform dark:bg-zinc-800" :class="activeTab === {{ $index }} ? 'rotate-180' : ''">
                                            <x-ui.icon.chevron-down class="size-3" />
                                        </div>
                                    </div>
                                </button>
                                    ...
                                    <div class="border-t border-zinc-100 p-4 pt-3 dark:border-zinc-700/50">
                                        ...
                                        <div class="mt-4 text-right">
                                            <a href="#" class="inline-flex items-center gap-1 text-[11px] font-bold text-blue-600 hover:text-blue-700 hover:underline dark:text-blue-400">
                                                View Report <x-ui.icon.arrow-right class="size-3" />
                                            </a>
                                        </div>
                                    </div>
                                </div>
