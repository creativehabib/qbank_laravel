<div>
    <div class="space-y-12">
        ...
        @if($subscriptions->isNotEmpty())
            <div>
                <h2 class="text-2xl font-bold mb-6 flex items-center justify-center gap-2 text-zinc-800 dark:text-zinc-200">
                    <x-ui.icon.sparkles class="size-6 text-amber-500" /> Pro Subscriptions
                </h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                    @foreach($subscriptions as $plan)
                        ...
                        @endphp
                        <x-ui.card class="relative flex flex-col {{ $borderClass }}">
                            @if($isActive)
                                <div class="absolute -top-3 left-0 right-0 flex justify-center">
                                    <span class="bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><x-ui.icon.check-badge class="size-4" /> Current Plan</span>
                                </div>
                            ...
                            <div class="flex-grow space-y-3 mb-6 text-sm text-zinc-600 dark:text-zinc-400">
                                <div class="flex items-start gap-2"><x-ui.icon.check-circle class="size-5 text-emerald-500 shrink-0"/> Access all Premium Model Tests</div>
                                <div class="flex items-start gap-2"><x-ui.icon.check-circle class="size-5 text-emerald-500 shrink-0"/> Detailed AI Explanations</div>
                                <div class="flex items-start gap-2"><x-ui.icon.check-circle class="size-5 text-emerald-500 shrink-0"/> Ad-Free Experience</div>
                                @if($plan->description)
                                    <div class="flex items-start gap-2"><x-ui.icon.check-circle class="size-5 text-emerald-500 shrink-0"/> {{ $plan->description }}</div>
                                @endif
                            </div>
                            @if($isActive)
                                <x-ui.button variant="filled" class="w-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 cursor-default">
                                    Current Plan
                                </x-ui.button>
                            @else
                                <x-ui.button href="{{ route('student.checkout', $plan->id) }}" variant="{{ $loop->iteration == 2 ? 'primary' : 'outline' }}" class="w-full">
                                    Subscribe Now
                                </x-ui.button>
                            @endif
                        </x-ui.card>
        ...
        @if($courses->isNotEmpty())
            <div class="mt-16">
                <h2 class="text-2xl font-bold mb-6 flex items-center justify-center gap-2 text-zinc-800 dark:text-zinc-200">
                    <x-ui.icon.academic-cap class="size-6 text-emerald-500" /> Special Courses
                </h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                    @foreach($courses as $course)
                        <x-ui.card class="flex flex-col sm:flex-row gap-6 p-0 overflow-hidden">
                            ...
                            @else
                                <div class="w-full sm:w-1/3 bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center min-h-[150px]">
                                    <x-ui.icon.academic-cap class="size-16 text-white/50" />
                                </div>
                            @endif
                            <div class="p-6 w-full sm:w-2/3 flex flex-col justify-between">
                                ...
                                <div class="mt-4 flex items-center justify-between">
                                    ...
                                    @if(auth()->user()->hasActivePackage($course->id))
                                        <x-ui.button variant="filled" class="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 cursor-default">Enrolled</x-ui.button>
                                    @else
                                        <x-ui.button href="{{ route('student.checkout', $course->id) }}" variant="primary">Enroll Now</x-ui.button>
                                    @endif
                                </div>
                            </div>
                        </x-ui.card>
