<div class="mx-auto max-w-2xl space-y-6">
    <div class="text-center">
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-500/20">
            <x-ui.icon.document-magnifying-glass class="size-8 text-emerald-600 dark:text-emerald-400" />
        </div>
    ...
    <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form data-page-submit.prevent="processOmr">
            <div
                x-data="{ isUploading: false, progress: 0 }"
                x-on:page-upload-start="isUploading = true"
                x-on:page-upload-finish="isUploading = false"
                x-on:page-upload-error="isUploading = false"
                x-on:page-upload-progress="progress = $event.detail.progress"
                ...
                <div class="text-center">
                    @if ($omrImage && in_array(strtolower($omrImage->extension()), ['png', 'jpg', 'jpeg']))
                        <div class="relative mx-auto h-64 w-48 overflow-hidden rounded-lg border border-zinc-200 shadow-sm">
                            ...
                            <div data-page-loading data-page-target="processOmr" class="absolute inset-0 bg-emerald-500/20 backdrop-blur-[2px]">
                        ...
                        <button type="button" data-page-click="$set('omrImage', null)" onchange="document.getElementById('omr-upload').value = ''" class="mt-4 text-sm font-semibold text-red-500 hover:text-red-600 transition">
                            ছবি পরিবর্তন করুন
                        </button>
                    ...
                    @elseif($omrImage)
                        <div class="relative mx-auto flex h-64 w-48 items-center justify-center overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50 shadow-sm dark:bg-zinc-800 dark:border-zinc-700">
                            <div class="text-center">
                                <x-ui.icon.document-check class="mx-auto size-10 text-emerald-500" />
                                <p class="mt-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">ফাইল সিলেক্ট হয়েছে</p>
                            </div>
                            <div data-page-loading data-page-target="processOmr" class="absolute inset-0 bg-emerald-500/20 backdrop-blur-[2px]"></div>
                        </div>
                        <button type="button" data-page-click="$set('omrImage', null)" onchange="document.getElementById('omr-upload').value = ''" class="mt-4 text-sm font-semibold text-red-500 hover:text-red-600 transition">
                            ছবি পরিবর্তন করুন
                        </button>
                    ...
                    @else
                        <x-ui.icon.photo class="mx-auto size-12 text-zinc-300 dark:text-zinc-600" />
                        <div class="mt-4 flex justify-center text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                            <label for="omr-upload" class="relative cursor-pointer rounded-md bg-transparent font-semibold text-emerald-600 focus-within:outline-none hover:text-emerald-500 dark:text-emerald-400">
                                ...
                                <input id="omr-upload" data-page-model="omrImage" type="file" accept=".jpg, .jpeg, .png" class="sr-only">
                            </label>
                        </div>
            ...
            <div class="mt-6">
                ...
                >
                    <span data-page-loading.remove data-page-target="processOmr" class="flex items-center gap-2">
                        <x-ui.icon.cpu-chip class="size-5" /> স্ক্যান করে রেজাল্ট দেখুন
                    </span>
                    <span data-page-loading data-page-target="processOmr" class="flex items-center gap-2">
                        <x-ui.icon.arrow-path class="size-5 animate-spin" /> OMR প্রসেস করছে...
                    </span>
                </button>
            </div>
        </form>
