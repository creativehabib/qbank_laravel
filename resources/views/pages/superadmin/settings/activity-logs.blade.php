<div class="max-w-7xl mx-auto pb-12">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <x-ui.heading size="xl">Activity Logs</x-ui.heading>
                <x-ui.subheading>Monitor system-wide user activity and actions.</x-ui.subheading>
            </div>
        </div>
    </div>
    ...
    <x-ui.card>
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
            <x-ui.heading size="lg">Recent Activity</x-ui.heading>

            <div class="flex items-center gap-3">
                <div class="w-full sm:w-64">
                    <x-ui.input data-page-model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Search logs..." />
                </div>

                @if($logs instanceof \Illuminate\Pagination\LengthAwarePaginator && $logs->total() > 0)
                    <x-ui.button data-page-click="clearLogs" data-page-confirm="আপনি কি নিশ্চিত যে সম্পূর্ণ লগ ডিলিট করতে চান?" size="sm" variant="danger" icon="trash">
                        Clear All
                    </x-ui.button>
                @endif
            </div>
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                <td class="py-3 px-4">
                                    <div class="flex items-center gap-2">
                                        @if($log->user)
                                            <x-ui.avatar size="sm" :name="$log->user->name" :initials="$log->user->initials()" :src="$log->user->picture ? asset('storage/'.$log->user->picture) : ''" />
                                ...
                                <td class="py-3 px-4 font-medium text-zinc-700 dark:text-zinc-300">
                                    <x-ui.badge size="sm" color="zinc">{{ str_replace('_', ' ', Str::title($log->action)) }}</x-ui.badge>
                                </td>
