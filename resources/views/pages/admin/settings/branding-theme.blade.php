<div class="max-w-7xl mx-auto pb-12">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <x-ui.heading size="xl">Branding & theme</x-ui.heading>
                <x-ui.subheading>Make the admin yours — app name, logos, accent color, favicon, and default theme.</x-ui.subheading>
            </div>
        </div>
        ...
        <div class="flex-1 space-y-6">
            ...
            <x-ui.card>
                ...
                <div class="space-y-2">
                    ...
                    <x-ui.input data-page-model="app_name" placeholder="Hyper POS" />
            ...
            <x-ui.card>
                ...
                <div class="space-y-2">
                    ...
                    <x-ui.input data-page-model="footer_text" placeholder="© 2026 Hyper POS. All rights reserved." />
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @foreach($colors as $color)
                                <button type="button" data-page-click="$set('accent_color', '{{ $color }}')"
                        ...
                        <div class="flex items-center">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <div class="relative flex items-center justify-center size-10 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-1 shadow-sm">
                                    ...
                                    <input type="color" data-page-model.live="accent_color" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                                </div>
                                <span class="text-sm font-medium text-zinc-600 dark:text-zinc-300 w-16">{{ strtoupper($accent_color) }}</span>
                            </label>
                            <button type="button" data-page-click="$set('accent_color', '#f97316')" class="text-sm font-medium text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors ml-4">
                                Reset to default
                            </button>
                        </div>
                    </div>
            ...
            <x-ui.card>
                ...
                <div class="flex items-center gap-4">
                    <div class="flex gap-2">
                        <button type="button" data-page-click="$set('text_color', '#ffffff')"
                        ...
                        <button type="button" data-page-click="$set('text_color', '#000000')"
                    ...
                    <label class="flex items-center gap-3 cursor-pointer ml-2">
                        <div class="relative flex items-center justify-center size-10 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-1 shadow-sm">
                            ...
                            <input type="color" data-page-model.live="text_color" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @foreach($darkColors as $color)
                                <button type="button" data-page-click="$set('dark_bg_color', '{{ $color }}')"
                                        class="w-8 h-8 rounded-lg shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-900 dark:focus:ring-offset-zinc-900
                        ...
                        <div class="flex items-center">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <div class="relative flex items-center justify-center size-10 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-1 shadow-sm">
                                    ...
                                    <input type="color" data-page-model.live="dark_bg_color" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                                </div>
                                <span class="text-sm font-medium text-zinc-600 dark:text-zinc-300 w-16">{{ strtoupper($dark_bg_color) }}</span>
                            </label>
                            <button type="button" data-page-click="$set('dark_bg_color', '#18181b')" class="text-sm font-medium text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors ml-4">
                                Reset to default
                            </button>
                        </div>
                    </div>
            ...
            <x-ui.card>
                ...
                <div class="space-y-2">
                    ...
                    <x-ui.select data-page-model="default_theme" class="w-full">
                        <x-ui.select.option value="System">System default</x-ui.select.option>
...
<script>
    document.addEventListener('page:initialized', () => {
        window.addEventListener('branding-saved', () => {
            if (window.AppUI) {
                window.AppUI.toast({ variant: 'success', text: 'Branding settings updated successfully!' });
            }
        });
    });
</script>
...
<script>
    document.addEventListener('page:init', () => {
        window.addEventListener('default-theme-updated', (event) => {
            let theme = event.theme || (event[0] && event[0].theme) || (event[0] ? event[0] : null);

            if (theme) {
                window.localStorage.removeItem('app.appearance');
                window.localStorage.removeItem('theme');
            }
        });

        window.addEventListener('branding-saved', () => {
            if (typeof AppUI !== 'undefined' && typeof AppUI.toast === 'function') {
                AppUI.toast({
