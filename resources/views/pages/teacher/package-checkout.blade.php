    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="mt-5 space-y-3">
                <button type="button" data-page-click="$set('paymentMethod', 'wallet')" class="w-full rounded-xl border p-4 text-left {{ $paymentMethod === 'wallet' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-zinc-300 dark:border-zinc-700' }}">
                ...
                @if($bkashEnabled)
                    <button type="button" data-page-click="$set('paymentMethod', 'bkash')" class="flex w-full items-center gap-3 rounded-xl border p-4 text-left {{ $paymentMethod === 'bkash' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-zinc-300 dark:border-zinc-700 hover:border-pink-500' }}">
                ...
                @if($nagadEnabled)
                    <button type="button" data-page-click="$set('paymentMethod', 'nagad')" class="flex w-full items-center gap-3 rounded-xl border p-4 text-left {{ $paymentMethod === 'nagad' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-zinc-300 dark:border-zinc-700 hover:border-orange-500' }}">
                ...
                @if($sslEnabled)
                    <button type="button" data-page-click="$set('paymentMethod', 'sslcommerz')" class="flex w-full items-center gap-3 rounded-xl border p-4 text-left {{ $paymentMethod === 'sslcommerz' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-zinc-300 dark:border-zinc-700 hover:border-blue-500' }}">
                ...
                <button type="button" data-page-click="$set('paymentMethod', 'manual')" class="flex w-full items-center justify-between rounded-xl border p-4 text-left {{ $paymentMethod === 'manual' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-zinc-300 dark:border-zinc-700' }}">
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            @if($paymentMethod === 'wallet' || $paymentMethod === 'manual')
                <button data-page-click="confirmPurchase" class="mt-5 w-full rounded-xl bg-indigo-600 px-4 py-3 font-semibold text-white hover:bg-indigo-700">Pay Now</button>
            ...
            @endif
            <a data-page-navigate href="{{ route('teacher.pricing') }}" class="mt-3 block text-center text-sm text-zinc-500 hover:text-zinc-700">Back to pricing</a>
        </div>
    </div>
</div>
