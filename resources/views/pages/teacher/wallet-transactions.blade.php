<div class="space-y-6">
    ...
    <div class="flex justify-center">
        <div class="inline-flex rounded-2xl border border-zinc-200 bg-white p-1">
            <button data-page-click="setTab('recharge')" class="rounded-xl px-4 py-2 text-sm font-semibold {{ $activeTab === 'recharge' ? 'bg-indigo-600 text-white' : 'text-zinc-600' }}">Recharge</button>
            <button data-page-click="setTab('withdraw')" class="rounded-xl px-4 py-2 text-sm font-semibold {{ $activeTab === 'withdraw' ? 'bg-indigo-600 text-white' : 'text-zinc-600' }}">Withdraw</button>
            <button data-page-click="setTab('report')" class="rounded-xl px-4 py-2 text-sm font-semibold {{ $activeTab === 'report' ? 'bg-indigo-600 text-white' : 'text-zinc-600' }}">Report</button>
        </div>
    </div>
        ...
        <div class="mx-auto max-w-3xl rounded-2xl border border-zinc-200 bg-white p-6">
            ...
            <div class="mt-5 flex flex-wrap gap-2">
                @foreach([20,50,100,200,500] as $preset)
                    <button type="button" data-page-click="$set('amount', {{ $preset }})" class="rounded-lg bg-zinc-100 px-4 py-2 text-sm font-semibold">৳{{ $preset }}</button>
                @endforeach
            </div>

            <label class="mt-5 block text-sm font-medium">Amount (৳)</label>
            <input type="number" data-page-model="amount" class="mt-2 w-full rounded-xl border border-zinc-300 px-4 py-3" placeholder="Enter amount" />

            <p class="mt-5 text-sm font-medium">Payment Method</p>
            <div class="mt-3 grid gap-3 md:grid-cols-3">
                <button data-page-click="setPaymentMethod('bkash')" type="button" class="rounded-2xl border p-5 text-center {{ $paymentMethod === 'bkash' ? 'border-blue-600 bg-blue-50' : 'border-zinc-300' }}"><p class="text-3xl font-extrabold text-pink-600">bKash</p><p class="mt-2 text-sm text-zinc-600">bKash</p></button>
                <button data-page-click="setPaymentMethod('nagad')" type="button" class="rounded-2xl border p-5 text-center {{ $paymentMethod === 'nagad' ? 'border-blue-600 bg-blue-50' : 'border-zinc-300' }}"><p class="text-3xl font-extrabold text-orange-600">Nagad</p><p class="mt-2 text-sm text-zinc-600">Nagad</p></button>
                <button data-page-click="setPaymentMethod('sslcommerz')" type="button" class="rounded-2xl border p-5 text-center {{ $paymentMethod === 'sslcommerz' ? 'border-blue-600 bg-blue-50' : 'border-zinc-300' }}"><p class="text-3xl font-extrabold text-blue-600">SSLCOMMERZ</p><p class="mt-2 text-sm text-zinc-600">All Cards</p></button>
            </div>

            @error('amount')<p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror
            <button data-page-click="submitRecharge" class="mt-5 w-full rounded-xl bg-indigo-600 py-3 text-lg font-bold text-white">Recharge Now</button>
        </div>
    @elseif($activeTab === 'withdraw')
        <div class="mx-auto max-w-3xl rounded-2xl border border-zinc-200 bg-white p-6">
            ...
            <div class="mt-2 grid gap-3 md:grid-cols-2">
                <button type="button" data-page-click="$set('withdrawBalanceType', 'reward')" class="rounded-xl border p-4 text-left {{ $withdrawBalanceType === 'reward' ? 'border-blue-600 bg-blue-50' : 'border-zinc-300' }}">Reward Balance<br><span class="text-sm text-zinc-500">Balance: ৳ {{ (int) $wallet->reward_balance }}</span></button>
                <button type="button" data-page-click="$set('withdrawBalanceType', 'credit')" class="rounded-xl border p-4 text-left {{ $withdrawBalanceType === 'credit' ? 'border-blue-600 bg-blue-50' : 'border-zinc-300' }}">Credit Balance<br><span class="text-sm text-zinc-500">Balance: ৳ {{ (int) $wallet->credit_balance }}</span></button>
            </div>
            ...
            <select data-page-model="paymentMethod" class="mt-2 w-full rounded-xl border border-zinc-300 px-4 py-3"><option value="bkash">bKash</option><option value="nagad">Nagad</option><option value="sslcommerz">SSLCOMMERZ</option></select>

            <label class="mt-4 block text-sm font-medium">Account Number</label>
            <input type="text" data-page-model="accountNumber" class="mt-2 w-full rounded-xl border border-zinc-300 px-4 py-3" placeholder="Enter account number" />

            <label class="mt-4 block text-sm font-medium">Notes (Optional)</label>
            <textarea data-page-model="notes" rows="3" class="mt-2 w-full rounded-xl border border-zinc-300 px-4 py-3" placeholder="Add any additional notes..."></textarea>
            @error('amount')<p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror
            <button data-page-click="submitWithdraw" class="mt-5 w-full rounded-xl bg-indigo-600 py-3 text-lg font-bold text-white">Submit Withdrawal Request</button>
        </div>
    @else
        <div class="space-y-4">
