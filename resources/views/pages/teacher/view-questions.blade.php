<div class="bangla flex lg:gap-5 justify-between">
    <div class="flex-1 max-w-4xl mx-auto">
        ...
        <div class="mb-2 sticky top-14 text-sm md:text-md flex gap-2 items-center bg-gray-50 rounded p-2 border justify-between z-10">
            ...
            <button data-page-click="saveSelection" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded">
        ...
        <div class="flex gap-2 justify-between my-4">
            ...
            @if (count($selectedQuestions) === $this->availableQuestions->count() && $this->availableQuestions->count() > 0)
                <button data-page-click="toggleSelectAll" class="shrink-0 border px-3 py-1 rounded border-red-500 text-red-600 flex items-center gap-1 font-semibold text-sm">Deselect All</button>
            @else
                <button data-page-click="toggleSelectAll" class="shrink-0 border px-2 rounded border-green-500 flex items-center gap-1 text-sm">Select All</button>
            @endif
        </div>
        ...
        <div class="relative">
            @forelse ($this->availableQuestions as $question)
                <div data-page-click="toggleSelection('{{ $question->id }}')"
                    ...
                    <div class="p-2">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <div class="flex gap-2 items-center text-xs flex-wrap">
                                ...
                                <button data-page-click.stop="toggleExplanation({{ $question->id }})" class="shrink-0 bg-green-600 hover:bg-green-500 text-white rounded-full px-1.5 py-0.5 text-[10px]">ব্যাখ্যা</button>
                            </div>
                        </div>
                        <div class="p-1">
    ...
    <div>
        <div class="hidden bg-white fixed top-0 right-0 z-[1000] overflow-y-auto lg:z-0 py-5 lg:py-0 lg:sticky lg:block lg:top-16 w-80 print:hidden h-screen overflow-y-scroll sidebar">
            <div class="px-3 lg:px-1 bangla">
                ...
                <!-- মূল সার্চ বার -->
                <div class="my-2.5">
                    <input type="text" data-page-model.live.debounce.300ms="searchKeyword" placeholder="প্রশ্নের ভেতর কিওয়ার্ড সার্চ..." class="bangla w-full rounded-md border-gray-300 border p-1.5 text-sm focus:ring-2 focus:ring-green-500">
                </div>
                ...
                <div class="my-4 rounded-lg border">
                    ...
                    <div class="p-2 space-y-2">
                        <select data-page-model.live="selectedClassId" class="w-full border rounded p-1.5 text-sm">
                        ...
                        <select data-page-model.live="selectedSubjectId" class="w-full border rounded p-1.5 text-sm">
                        ...
                        <select data-page-model.live="selectedChapterId" class="w-full border rounded p-1.5 text-sm">
                        ...
                        <div class="p-2 pt-1 space-y-1 max-h-40 overflow-y-auto">
                            @foreach($availableExamCategories as $id => $name)
                                <label class="px-2 py-0.5 flex items-center cursor-pointer rounded transition-colors {{ in_array($id, $activeExamCategoryIds) ? 'bg-purple-100 font-bold text-purple-800 border border-purple-400' : 'hover:bg-purple-50 text-gray-700' }}"
                                       ...
                                       x-cloak>
                                    <input data-page-model.live="selectedExamCategories" value="{{ $id }}" type="checkbox" class="mr-2 rounded text-purple-600">
                    ...
                    <div class="p-2 space-y-1">
                        @foreach(['easy' => 'সহজ', 'medium' => 'মাধ্যম', 'hard' => 'কঠিন'] as $key => $label)
                            <label class="px-2 py-0.5 flex items-center cursor-pointer rounded hover:bg-slate-100">
                                <input data-page-model.live="selectedDifficulties" value="{{ $key }}" type="checkbox" class="mr-2 rounded">
                ...
                <div class="my-4 rounded-lg border">
                    ...
                    <div class="p-2 space-y-1">
                        @foreach(['mcq' => 'MCQ', 'cq' => 'CQ (সৃজনশীল)', 'short' => 'Short Question', 'written' => 'Written'] as $key => $label)
                            <label class="px-2 py-0.5 flex items-center cursor-pointer rounded hover:bg-slate-100">
                                <input data-page-model.live="selectedTypes" value="{{ $key }}" type="checkbox" class="mr-2 rounded">
                    ...
                    <div class="my-4 rounded-lg border">
                        ...
                        <div class="p-2 space-y-1 max-h-48 overflow-y-auto">
                            @foreach($availableTopics as $topicId => $topicName)
                                <label class="px-2 py-0.5 flex items-center cursor-pointer rounded hover:bg-slate-100">
                                    <input data-page-model.live="selectedTopics" value="{{ $topicId }}" type="checkbox" class="mr-2 rounded">
                        ...
                        <div class="p-2 pt-1 space-y-1 max-h-60 overflow-y-auto">
                            @foreach($availableTags as $id => $name)
                                <label class="px-2 py-0.5 flex items-center cursor-pointer rounded transition-colors {{ in_array($id, $activeTagIds) ? 'bg-green-100 font-bold text-green-800 border border-green-400' : 'hover:bg-slate-100 text-gray-700' }}"
                                       ...
                                       x-cloak>
                                    <input data-page-model.live="selectedTags" value="{{ $id }}" type="checkbox" class="mr-2 rounded">
