        ...
        @if($plans->isNotEmpty())
            <div>
                <h2 class="text-2xl font-bold mb-6 flex items-center justify-center gap-2 text-zinc-800 dark:text-zinc-200">
                    <x-ui.icon.briefcase class="size-6 text-indigo-500" /> Educator Subscriptions
                </h2>
                        ...
                        <x-ui.card class="relative flex flex-col {{ $borderClass }}">
                            ...
                            <div class="flex-grow space-y-3 mb-6 text-sm text-zinc-600 dark:text-zinc-400">
                                <div class="flex items-start gap-2">
                                    <x-ui.icon.document-plus class="size-5 text-indigo-500 shrink-0"/>
                                    <span><strong>{{ $plan->question_create_limit ?: 'Unlimited' }}</strong> Question Creations</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <x-ui.icon.eye class="size-5 text-indigo-500 shrink-0"/>
                                    <span><strong>{{ $plan->page_view_limit ?: 'Unlimited' }}</strong> Page Views</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <x-ui.icon.bolt class="size-5 text-indigo-500 shrink-0"/>
                                    <span>{{ $plan->is_ad_free ? 'Ad-Free Experience' : 'Contains Ads' }}</span>
                                </div>
                                @if($plan->description)
                                    <div class="flex items-start gap-2">
                                        <x-ui.icon.check-circle class="size-5 text-emerald-500 shrink-0"/>
                            ...
                            @else
                                <x-ui.button href="{{ route('teacher.pricing.checkout', $plan->id) }}" variant="{{ $loop->iteration == 2 ? 'primary' : 'outline' }}" class="w-full">
                                    Subscribe Now
                                </x-ui.button>
                            @endif
                        </x-ui.card>
