            ...
            <div class="p-5 xl:p-8 relative">
                ...
                @if(!$photo && !$scannedImageUrl)
                    <div x-data="{ isDropping: false }"
                         ...
                         x-on:drop.prevent="isDropping = false; $page.upload('photo', $event.dataTransfer.files[0])"
                        ...
                        <input type="file" data-page-model="photo" accept="image/*,.pdf" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                ...
                @else
                    <div class="relative bg-zinc-50 rounded-xl p-4 border border-zinc-200 flex justify-center min-h-[300px] overflow-hidden">
                        <button data-page-click="removePhoto" class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 z-30">
                        ...
                        <div data-page-loading data-page-target="scanOmr" class="absolute inset-0 z-20 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                ...
                <div class="mt-6 text-center flex justify-center">
                    <button data-page-click="scanOmr"
                        ...
                        <span data-page-loading.remove data-page-target="scanOmr">Scan and Match OMR</span>
                        <span data-page-loading data-page-target="scanOmr">Scanning... Please wait</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
            <div class="bg-zinc-50 border-b border-zinc-200 px-5 py-4 flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <select data-page-model="negativeMark" class="bg-white border border-zinc-200 rounded-lg px-3 py-1.5 text-xs font-medium focus:border-green-500 outline-none">
                    ...
                    <div class="flex items-center justify-end break-inside-avoid mb-3">
                        ...
                        <div class="flex gap-2">
                            @foreach(['A' => 'ক', 'B' => 'খ', 'C' => 'গ', 'D' => 'ঘ'] as $key => $label)
                                ...
                                @endphp
                                <button data-page-click="toggleAnswer({{ $i }}, '{{ $key }}')"
            ...
            <div class="flex items-center justify-between p-6 border-t border-zinc-200 bg-zinc-50">
                <div class="flex items-center gap-3">
                    ...
                    <input type="number" data-page-model.live="totalQuestions" class="w-24 px-2 py-1 bg-white border border-zinc-200 rounded-lg text-center font-bold outline-none focus:border-green-500" min="10" max="200">
                </div>
                <button data-page-click="saveAnswers" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-8 rounded-lg transition-colors flex items-center gap-2">
