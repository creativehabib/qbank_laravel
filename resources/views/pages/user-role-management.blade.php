<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <x-ui.heading size="xl" class="!font-semibold">Users</x-ui.heading>
            <x-ui.text class="mt-1 !text-sm">The people who can sign in. Give each a role per store.</x-ui.text>
        </div>

        <div class="flex items-center gap-3">
            <x-ui.dropdown>
                <x-ui.button variant="ghost" icon="arrow-down-tray" icon-trailing="chevron-down" class="!text-zinc-600 dark:!text-zinc-400">Export</x-ui.button>
                <x-ui.menu>
                    <x-ui.menu.item icon="document-text">Export as CSV</x-ui.menu.item>
                </x-ui.menu>
            </x-ui.dropdown>
            <x-ui.button data-page-click="createUser" variant="primary" icon="plus">
                New user
            </x-ui.button>
        </div>
    </div>
        ...
        <div class="px-4 py-3 border-b border-zinc-200 dark:border-zinc-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4 w-full sm:w-auto">
                ...
                <div class="w-full sm:w-64">
                    <x-ui.input
                        data-page-model.live.debounce.300ms="search"
            ...
            <div class="flex items-center gap-2">
                <x-ui.dropdown>
                    ...
                    <x-ui.menu class="w-48">
                        <x-ui.menu.radio.group data-page-model.live="sortOrder">
                            ...
                            <x-ui.menu.separator />
                            <x-ui.menu.radio value="id_asc">ID (lowest first)</x-ui.menu.radio>
                            <x-ui.menu.radio value="id_desc">ID (highest first)</x-ui.menu.radio>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-zinc-50 dark:bg-zinc-800/50 text-zinc-400 dark:text-zinc-500 text-[10px] uppercase font-bold tracking-wider">
                    <tr>
                        <th class="px-4 py-3 flex items-center gap-1">USER <x-ui.icon.chevron-up class="size-3" /></th>
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse($users as $user)
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <x-ui.avatar size="sm" :initials="$user->initials()" class="border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300" />
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($user->id !== auth()->id())
                                        <!-- Mock Toggle Switch -->
                                        <button type="button" data-page-click="toggleStatus({{ $user->id }})" class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" role="switch" aria-checked="{{ $isActive ? 'true' : 'false' }}">
                                            <span class="sr-only">Toggle status</span>
                                            <span aria-hidden="true" class="pointer-events-none absolute h-full w-full rounded-md bg-white"></span>
                                            <span aria-hidden="true" class="pointer-events-none absolute mx-auto h-4 w-9 rounded-full {{ $isActive ? 'bg-emerald-500' : 'bg-gray-200 dark:bg-zinc-700' }} transition-colors duration-200 ease-in-out"></span>
                                    ...
                                    <x-ui.dropdown align="end" offset="-10">
                                        <x-ui.button variant="ghost" size="sm" icon="ellipsis-vertical" class="!text-zinc-400 hover:!text-zinc-900 dark:hover:!text-zinc-100" />

                                        <x-ui.menu>
                                            <x-ui.menu.item icon="pencil" data-page-click="editUser({{ $user->id }})">Edit</x-ui.menu.item>
                                            <x-ui.menu.item icon="lock-closed" data-page-click="signOutEverywhere({{ $user->id }})">Sign out everywhere</x-ui.menu.item>

                                            @if($user->id !== auth()->id())
                                                <x-ui.menu.separator />
                                                <x-ui.menu.item icon="trash" variant="danger" x-on:click="window.confirmDeleteAction(() => $page.deleteUser({{ $user->id }}))">Delete</x-ui.menu.item>
                                            @endif
                                        </x-ui.menu>
                                    </x-ui.dropdown>
                                </div>
    ...
    <x-ui.modal data-page-model="showEditModal" class="md:w-[620px] max-w-2xl space-y-6">
        <div>
            <x-ui.heading size="lg">{{ $editingUserId ? 'Edit User' : 'Create User' }}</x-ui.heading>
            <x-ui.text class="mt-2 text-sm text-zinc-500">
        ...
        <div class="grid gap-4">
            ...
            @if(!$editingUserId)
                ...
                <x-ui.input
                    data-page-model="password_confirmation"
                    type="password"
        ...
        <div class="flex justify-end gap-2">
            <x-ui.button data-page-click="$set('showEditModal', false)" variant="ghost">Cancel</x-ui.button>
            <x-ui.button data-page-click="saveUser" variant="primary">{{ $editingUserId ? 'Update' : 'Create' }}</x-ui.button>
        </div>
    </x-ui.modal>
</div>
