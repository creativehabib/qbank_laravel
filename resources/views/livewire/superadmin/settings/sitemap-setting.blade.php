<div class="max-w-7xl mx-auto space-y-6 pb-12">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <flux:heading size="xl">XML Sitemap</flux:heading>
                <flux:subheading>Generate and manage the sitemap to help search engines crawl your site.</flux:subheading>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <flux:card>
                <div class="flex items-start gap-4">
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-500">
                        <flux:icon name="map" variant="outline" class="h-6 w-6" />
                    </div>
                    <div class="flex-1">
                        <flux:heading size="lg">Sitemap Generator</flux:heading>
                        ...
                        <div class="mt-6 flex flex-wrap gap-4">
                            <flux:button wire:click="generateSitemap" variant="primary" icon="arrow-path" wire:loading.attr="disabled">
                                <span wire:loading.remove wire:target="generateSitemap">Generate Sitemap Now</span>
                                <span wire:loading wire:target="generateSitemap">Generating... please wait</span>
                            </flux:button>
                            
                            @if($sitemapExists)
                                <flux:button href="{{ url('sitemap.xml') }}" target="_blank" variant="outline" icon="arrow-top-right-on-square">
                                    View Sitemap
                                </flux:button>
                            @endif
                        </div>
            ...
            <flux:card>
                <div class="flex flex-col gap-4">
                    <div class="flex items-center gap-2">
                        <flux:icon name="information-circle" class="w-6 h-6 text-accent" />
                        <flux:heading size="lg">কীভাবে সাইটম্যাপটি ব্যবহার করবেন? (Instructions)</flux:heading>
                    </div>
                            ...
                            <div x-data="{ 
                                ...
                                copy() {
                                    ...
                                    if (typeof window.Flux !== 'undefined') {
                                        window.Flux.toast({ variant: 'success', text: 'Copied to clipboard!' });
                                    }
                                }
                                ...
                                <button @click="copy()" type="button" class="shrink-0 p-1.5 rounded hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 focus:outline-none" title="Copy">
                                    <div x-show="!copied"><flux:icon name="clipboard-document" class="w-4 h-4" /></div>
                                    <div x-show="copied" x-cloak><flux:icon name="check" class="w-4 h-4 text-emerald-500" /></div>
                                </button>
                            </div>
                        </div>
        ...
        <div class="space-y-6">
            <flux:card>
                <flux:heading size="md" class="mb-4">Sitemap Status</flux:heading>
                ...
                @else
                    <div class="py-6 text-center text-zinc-500 flex flex-col items-center justify-center">
                        <flux:icon name="document-magnifying-glass" class="h-8 w-8 mb-2 opacity-50" />
            ...
            <flux:card>
                <flux:heading size="md" class="mb-2">Search Engines</flux:heading>
