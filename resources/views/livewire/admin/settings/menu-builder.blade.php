<div class="max-w-7xl mx-auto space-y-6 pb-12">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <flux:heading size="xl">Premium Menu Builder</flux:heading>
                <flux:subheading>Drag & Drop to arrange menus for Main Navbar, Footer, and Mobile App.</flux:subheading>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1 space-y-2">
            @foreach($menus as $menu)
                ...
                >
                    ...
                    <flux:icon name="chevron-right" class="w-4 h-4 opacity-50" />
                </button>
            @endforeach
        </div>
        ...
        <div class="lg:col-span-3">
            ...
            <flux:card>
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <flux:heading size="lg">{{ $this->activeMenu->name }}</flux:heading>
                        <flux:text class="text-sm mt-1">Drag items to reorder them.</flux:text>
                    ...
                    <flux:button wire:click="openItemModal" variant="primary" icon="plus">Add Item</flux:button>
                </div>
                ...
                @if($this->activeMenu->parentItems->isEmpty())
                    <div class="py-12 text-center text-zinc-500 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700">
                        <flux:icon name="queue-list" class="w-10 h-10 mx-auto mb-3 opacity-50" />
                        <p>No items in this menu yet.</p>
                        <flux:button wire:click="openItemModal" variant="outline" size="sm" class="mt-4">Add your first item</flux:button>
                    </div>
                @else
                    <!-- Sortable List -->
                    <div 
                                ...
                                <li data-id="{{ $item->id }}" class="bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-sm overflow-hidden">
                                    <div class="flex items-center p-3 gap-3">
                                        <div class="drag-handle cursor-grab active:cursor-grabbing text-zinc-400 hover:text-zinc-600 transition-colors">
                                            <flux:icon name="bars-2" class="w-5 h-5" />
                                        </div>
                                        <div class="flex-1">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <flux:button wire:click="editItem({{ $item->id }})" variant="ghost" size="sm" icon="pencil-square" class="text-zinc-500 hover:text-accent" />
                                            <flux:button wire:click="deleteItem({{ $item->id }})" wire:confirm="Are you sure you want to delete this item?" variant="ghost" size="sm" icon="trash" class="text-red-500 hover:text-red-600 dark:text-red-400" />
                                        </div>
                                    </div>
                                            ...
                                            @foreach($item->children as $child)
                                                <li class="flex items-center justify-between bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 p-2 rounded">
                                                    ...
                                                    <div class="flex gap-1">
                                                        <flux:button wire:click="editItem({{ $child->id }})" variant="ghost" size="sm" icon="pencil-square" class="h-6 w-6 text-zinc-500 p-0" />
                                                        <flux:button wire:click="deleteItem({{ $child->id }})" wire:confirm="Are you sure?" variant="ghost" size="sm" icon="trash" class="h-6 w-6 text-red-500 p-0" />
                                                    </div>
                                                </li>
                                            @endforeach
                                        </ul>
    ...
    <flux:modal wire:model="showItemModal" class="md:w-[500px]">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $editingItemId ? 'Edit Menu Item' : 'Add Menu Item' }}</flux:heading>
            </div>

            <div class="space-y-4">
                <flux:input wire:model="itemTitle" label="Link Title" placeholder="e.g. Home" required />
                
                <flux:input wire:model="itemUrl" label="URL (Link)" placeholder="e.g. /job-solutions or https://google.com" />
                
                @if($this->activeMenu && $this->activeMenu->parentItems->count() > 0 && !$editingItemId)
                <flux:select wire:model="itemParentId" label="Parent Item (Optional)">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="itemIcon" label="Icon Class (Optional)" placeholder="e.g. fas fa-home" />
                    <flux:input wire:model="itemBadge" label="Badge Text (Optional)" placeholder="e.g. নতুন / New" />
                </div>
                <div>
                    <flux:select wire:model="itemTarget" label="Open In">
